...
/*   Updated: 2023/10/10 12:19:59 by frcastil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_check_correct_digits_2(char **str)
{
	int	i;
	int	j;
	...
	while (str[i] != NULL)
	{
		...
		if (ft_atoi(str[i]) < MIN_INT || ft_atoi(str[i]) > MAX_INT)
			return (FALSE);
		i++;
	}
	return (TRUE);
...
void	ft_check_conditions_2(char **str)
{
	...
	if (ft_check_correct_digits_2(str) == FALSE)
	{
		ft_printf("Error\nThere are not correct digits\n");
		exit(EXIT_FAILURE);
	}
